...
const LinkComment = ({comment, link, setLink}) => {
    ...
    const postedByAuthor = user && user.uid === comment.postedBy.id;
    ...
    function handleEditComment(commentText) {
        ...
        linkRef.get().then(doc => {
            if (doc.exists) {
                ...
                const updatedComments = prevComments.map(item => item.created === comment.created ? newComment : item);
    ...
    function handleDeleteComment() {
        ...
        linkRef.get().then(doc => {
            if (doc.exists) {
                const prevComments = doc.data().comments;
                const updatedComments = prevComments.filter(item => item.created !== comment.created);
                linkRef.update({comments: updatedComments});
                setLink((prevState) => ({
                    ...prevState,
                    comments: updatedComments
    ...
    return (
        <>
            <CommentModal isOpen={showModal} title="Edit comment" sendAction={handleEditComment}
                closeAction={handleCloseModal} comment={comment} />
                <IonCard>
                    <IonCardContent>
                        <IonList lines="none">
                            <IonItem>
